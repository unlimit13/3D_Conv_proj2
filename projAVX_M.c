...
void *woker(void *arg){
    ...
    for(int k=0;k<data->row_length;k++){
        for(int m=0;m<kernel_height;m++){
            ...
            for(int o=0;o<kernel_height;o++){
                for(int p=0;p<kernel_height;p++){
                    input_temp[o][p] = data->input[data->i+m][data->j+o][k+p];
            ...
            for(int o=0;o<kernel_height;o++){
                for(int v=0;v<kernel_height;v++){
                    data->output[data->i][data->j][k] += values[o][v];
                }
            }
            free(input_temp);
            free(kernel_temp);
    ...
    for(int i=0;i<height_length;i++){
        for(int j=0;j<col_length;j++){
                ...
                //printf("here1!\n");
                    ...
                    thread_params->i = i;
                    thread_params->j = j;
                    thread_params->row_length = row_length;
                    thread_params->kernel_size = kernel_height;
                    //printf("here3!\n");
                    //printf("m : %d\n",m);
                    pthread_create(&threads[j],NULL,woker,(void*)thread_params);
